...
const DEFAULT_PRODUCTS = [
  ...
  'NARANJA','MANZANA','MANDARINA','TOMATE','TOMATE PREMIUM','HABANERO','AUYAMA','PAPAYA','JUDIA','CHAYOTE','OKRA','YAUTIA','GUINDILLA VERDE FINA','AUYAMA'
];
    ...
    <section class="grid-2" style="margin-top:12px">
      <article class="card">
        ...
        <div class="toolbar" style="margin-top:12px">
          ...
          <button class="btn btn-soft" type="button" id="printSummaryDashBtn">PDF resumen</button>
        </div>
      </article>
      <article class="card">
    ...
    <section class="card" style="margin-top:12px">
      ...
      <div class="client-list">
        ${activeClients().length ? activeClients().sort((a,b)=>a.name.localeCompare(b.name,'es')).map(client => {
          ...
          return `<article class="client-card"><div class="client-top"><div><h3>${escapeHtml(client.name)}</h3><div class="tiny">${escapeHtml(client.phone || 'Sin teléfono')} · ${client.commissionPct ?? state.settings.defaultCommissionPct}%</div></div><span class="status-pill ${statusClass(pay.status)}">${pay.status.toUpperCase()}</span></div><div class="tiny-actions" style="margin-top:10px"><span class="badge">${stats.totalLines} líneas</span><span class="badge">${qtyFormat(stats.totalQty)} uds</span><span class="badge">Base ${eur(pay.base)}</span><span class="badge">Pendiente ${eur(pay.pending)}</span></div><div class="toolbar" style="margin-top:12px"><button class="btn btn-soft btn-xs" type="button" data-open-orders="${client.id}">Abrir pedido</button><button class="btn btn-soft btn-xs" type="button" data-open-payments="${client.id}">Cobro</button><button class="btn btn-soft btn-xs" type="button" data-print-client="${client.id}">PDF</button><button class="btn btn-warn btn-xs" type="button" data-share-client="${client.id}">WhatsApp</button><button class="btn btn-danger btn-xs" type="button" data-remove-client="${client.id}">Eliminar</button></div></article>`;
  ...
  root.innerHTML = `
    ...
    ${selected ? `
      <section class="order-layout" style="margin-top:12px">
        ...
        <article class="card">
          <div class="section-title"><div><h2>${escapeHtml(selected.name)}</h2><div class="subline">${orderStats(selected.id).totalLines} líneas · ${qtyFormat(orderStats(selected.id).totalQty)} uds</div></div><div class="toolbar"><button class="btn btn-soft btn-xs" type="button" data-print-client="${selected.id}">PDF</button><button class="btn btn-warn btn-xs" type="button" data-share-client="${selected.id}">WhatsApp</button><button class="btn btn-danger btn-xs" type="button" data-clear-order="${selected.id}">Vaciar</button></div></div>
...
function renderProvider(){
  ...
  root.innerHTML = `
    <section class="card">
      <div class="section-title"><div><h2>Proveedor</h2><div class="subline">Total, detalle por cliente y faltantes</div></div><div class="toolbar"><button class="btn btn-soft btn-xs" type="button" id="shareProviderTotalBtn">WhatsApp total</button><button class="btn btn-soft btn-xs" type="button" id="copyProviderTotalBtn">Copiar</button><button class="btn btn-soft btn-xs" type="button" id="printProviderBtn">PDF actual</button></div></div>
...
function renderMore(){
  ...
  root.innerHTML = `
    <section class="grid-2">
      ...
      <article class="editor-card">
        <div class="section-title"><div><h2>PDF, historial y limpieza</h2><div class="subline">Exportaciones rápidas y resumen antes de limpiar el día</div></div></div>
        <div class="toolbar"><button class="btn btn-soft" type="button" id="printSummaryBtn">PDF resumen</button><button class="btn btn-soft" type="button" id="printProviderTotalMoreBtn">PDF proveedor total</button><button class="btn btn-soft" type="button" id="printProviderClientsMoreBtn">PDF por cliente</button><button class="btn btn-soft" type="button" id="printMissingMoreBtn">PDF faltantes</button><button class="btn btn-soft" type="button" id="printAllClientsMoreBtn">PDF todos clientes</button><button class="btn btn-soft" type="button" id="copyMissingBtn">Copiar faltantes</button><button class="btn btn-danger" type="button" id="clearDayBtn">Limpiar día</button></div>
...
function buildClientPrintSection(clientId){
  const client = getClient(clientId);
  if(!client) return '';
  const lines = state.orders[clientId] || [];
  const pay = computePayment(clientId);
  const rows = lines.map(line => `<tr><td>${escapeHtml(line.product)}</td><td>${qtyFormat(line.qty)}</td><td>${escapeHtml(line.unit||state.settings.defaultUnit)}</td><td>${escapeHtml(line.note||'')}</td></tr>`).join('');
  return `<h2>${escapeHtml(client.name)}</h2><table class="table"><thead><tr><th>Producto</th><th>Cantidad</th><th>Unidad</th><th>Nota</th></tr></thead><tbody>${rows || '<tr><td colspan="4">Sin pedido</td></tr>'}</tbody></table><div class="box"><div class="row"><span>Base proveedor</span><strong>${eur(pay.base)}</strong></div><div class="row"><span>Mi comisión</span><strong>${eur(pay.mine)}</strong></div><div class="row"><span>Total cobrar</span><strong>${eur(pay.total)}</strong></div><div class="row"><span>Pagado</span><strong>${eur(pay.paid)}</strong></div><div class="row"><span>Pendiente</span><strong>${eur(pay.pending)}</strong></div></div>`;
}

function printClient(clientId){
  const client = getClient(clientId);
  if(!client) return;
  openPrintable(`Cliente ${client.name}`, `<h1>${escapeHtml(client.name)}</h1><small>${niceDate(state.dayKey)}</small>${buildClientPrintSection(clientId).replace(`<h2>${escapeHtml(client.name)}</h2>`, '')}`);
}

function printAllClients(){
  const content = activeClients().map(client => buildClientPrintSection(client.id)).join('');
  openPrintable('Todos los clientes', `<h1>${escapeHtml(state.settings.appName)}</h1><small>Clientes · ${niceDate(state.dayKey)}</small>${content || '<p>Sin clientes</p>'}`);
}

function printProviderTotal(){
  const rows = providerGroups().total.map(item => `<tr><td>${escapeHtml(item.product)}</td><td>${qtyFormat(item.qty)}</td><td>${escapeHtml(item.unit)}</td><td>${statusLabel(item.status)}</td></tr>`).join('');
  openPrintable('Proveedor total', `<h1>${escapeHtml(state.settings.appName)}</h1><small>Proveedor total · ${niceDate(state.dayKey)}</small><table class="table"><thead><tr><th>Producto</th><th>Cantidad</th><th>Unidad</th><th>Estado</th></tr></thead><tbody>${rows || '<tr><td colspan="4">Sin datos</td></tr>'}</tbody></table>`);
}

function printProviderClients(){
  const content = activeClients().map(client => {
    const lines = state.orders[client.id] || [];
    const rows = lines.map(line => `<tr><td>${escapeHtml(line.product)}</td><td>${qtyFormat(line.qty)}</td><td>${escapeHtml(line.unit||state.settings.defaultUnit)}</td><td>${escapeHtml(line.note||'')}</td></tr>`).join('');
    return `<h2>${escapeHtml(client.name)}</h2><table class="table"><thead><tr><th>Producto</th><th>Cantidad</th><th>Unidad</th><th>Nota</th></tr></thead><tbody>${rows || '<tr><td colspan="4">Sin pedido</td></tr>'}</tbody></table>`;
  }).join('');
  openPrintable('Proveedor por cliente', `<h1>${escapeHtml(state.settings.appName)}</h1><small>Proveedor por cliente · ${niceDate(state.dayKey)}</small>${content || '<p>Sin clientes</p>'}`);
}

function printMissing(){
  const combined = [...providerGroups().unavailable, ...providerGroups().external];
  const content = combined.map(item => `<h2>${escapeHtml(item.product)} · ${qtyFormat(item.qty)} ${escapeHtml(item.unit)}</h2><div class="box"><div class="row"><span>Estado</span><strong>${statusLabel(item.status)}</strong></div></div><table class="table"><thead><tr><th>Cliente</th><th>Cantidad</th><th>Unidad</th><th>Nota</th></tr></thead><tbody>${item.clients.map(c => `<tr><td>${escapeHtml(c.clientName)}</td><td>${qtyFormat(c.qty)}</td><td>${escapeHtml(item.unit)}</td><td>${escapeHtml(c.note||'')}</td></tr>`).join('')}</tbody></table>`).join('');
  openPrintable('Faltantes y externos', `<h1>${escapeHtml(state.settings.appName)}</h1><small>Faltantes / externos · ${niceDate(state.dayKey)}</small>${content || '<p>Sin faltantes ni externos</p>'}`);
}

function printCurrentProviderTab(){
  if(providerTab === 'clientes') return printProviderClients();
  if(providerTab === 'faltantes') return printMissing();
  return printProviderTotal();
}
...
document.addEventListener('click', (e) => {
  ...
  if(e.target.id === 'printSummaryBtn' || e.target.id === 'printSummaryDashBtn'){ printSummary(); return; }
  if(e.target.id === 'printProviderBtn' || e.target.id === 'printProviderTotalMoreBtn'){ providerTab = e.target.id === 'printProviderTotalMoreBtn' ? 'total' : providerTab; printCurrentProviderTab(); return; }
  if(e.target.id === 'printProviderClientsMoreBtn'){ printProviderClients(); return; }
  if(e.target.id === 'printMissingMoreBtn'){ printMissing(); return; }
  if(e.target.id === 'printAllClientsMoreBtn'){ printAllClients(); return; }
  ...
  const printClientBtn = e.target.closest('[data-print-client]'); if(printClientBtn){ printClient(printClientBtn.dataset.printClient); return; }
